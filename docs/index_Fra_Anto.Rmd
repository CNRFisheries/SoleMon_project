...
- **WindDirection**: follow the code on the sheet "Notes for compilation"
- **SeaCondition**: follow the code on the sheet "Notes for compilation"
- **WeatherCond**: follow the code on the sheet "Notes for compilation"
...
To start a new file, copy the template version called "Maschera inserimento SOLEMON_template.accdb", paste it in the same "OnBoard/access" (or OnBoard_2024_PLE/access) subfolder and rename it as "Maschera inserimento SOLEMON_YEAR_N.accdb", where YEAR is the reporting year and N is the name of the computer you are usinf (FRA or ENRI). Example: "**Masschera inserimento SOLEMON_2024_FRA.accdb**" . *It does not matter if parts of the same haul are in different files, this is handled in post-processing. It does not matters if access with the same name are located in different laptops, this is handled in post processing*.
...
-   **kg_field3**: weight in kilograms, placeholder for cumulative number and subsamples. The compilation depends on the case
