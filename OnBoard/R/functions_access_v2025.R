...
function1=function(haul, db=NA, year, complete=F){
  ...
  print(paste('Processing haul', haul, collapse = "\n"))
        ...
        if(subsamples_target[k,]$type_subsample=='C1_species'){
          ...
          if(!is.na(subsamples_target[k,]$kg_field2)&subsamples_target[k,]$kg_field2!=0){
            print(paste('ratio between recorded and measured subsample is:', 
                        (round((meas_grams/1000)*100/subsamples_target[k,]$kg_field1,2)),"%"))
...
function_benthos=function(haul, db=NA, year, complete=F){
  # to be done
  print(paste('Processing haul', haul, " benthos", collapse = "\n"))
  ...
  deleterow=xdat[xdat$length_mm %in% c(NA, 0) & 
                   ...
                   is.na(xdat$type_subsample)  &
                   xdat$total_number %in% c(NA, 0)&
                   xdat$subsample_number %in% c(NA, 0)&
                   xdat$kg_field3 %in% c(NA, 0)&
                   xdat$Sex %in% c(NA, 0),]$idfk
  ...
  if(nrow(subs.nt)>0){
    ...
    subs.nt=subs.nt%>%
      dplyr::group_by(gear, species_name)%>%
      dplyr::summarise(w=0, n=sum(total_number))
  }
  ...
  weight_not_target=xdat[xdat$target!=1,]%>%
    dplyr::group_by(gear, species_name)%>%
  ...
  if(nrow(lioc_data)>0&is.na(lioc_data$type_subsample[1])){
  ...
  }else if(nrow(lioc_data)>0&lioc_data$type_subsample[1]=='C1_species'){
    # case 1, presi tutti animali da cala e poi fatto subcampione per ottenere il numero: 
    # n va calcolato, poi kg_subsample diventa w_g
    print(paste('subsample case1 done for LIOCDEP'))
    if(!is.na(lioc_data[1,]$kg_field2)){
      print(paste('ratio between recorded and measured subsample is:', 
                  (lioc_data[1,]$kg_field2*1000)/(lioc_data[1,]$kg_field1*1000)))
      meas_grams=lioc_data[1,]$kg_field2*1000
      meas_n=nrow(lioc_data)
      # assign number and weight
      lioc_data[1,]$total_number=round((lioc_data[1,]$kg_field1*1000)*(meas_n/meas_grams))
      lioc_data[1,]$weight_g=round(lioc_data[1,]$kg_field1*1000)
      weight_lioc=lioc_data[1,]%>%mutate(w=weight_g/1000,n=total_number)%>%
        select(gear,species_name,w,n)
      weight_not_target=rbind(weight_not_target, weight_lioc)
    }
    }
...
# performs checks ####
#**********************************
function2=function(xdat, haul, year){
  ...
  p_lw<-ggplot(data=xdat_target_2%>%filter(weight_g>0))+
 ...
 ggsave(plot=p_lw, filename = file.path(main_wd, 'output', 'checks', paste0(haul, '_targetlw.png') ), width = 30, height = 20, units='cm')
 
 target_suspicious<-xdat_target_2%>%mutate(suspicious=ifelse(weight_g>hi.ci|weight_g<lo.ci,1,0))%>%filter(suspicious==1)
 
 if(nrow(target_suspicious)>0){
 write.csv(target_suspicious,file.path(main_wd, 'output', 'checks', paste0(haul, '_targetlw_suspicious.csv')),row.names=F)}
  ...
  # trend number of samples with rapido A,D and cobined (C)#### 
